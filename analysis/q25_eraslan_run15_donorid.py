"""Q25 / Run 15 — Eraslan paired with same-donor z_bio MSE matching.

Key new term:
  loss_donor_id = MSE(z_bio_bulk_d, z_bio_sc_d)

For each paired (bulk_d, sc_d), force z_bio of the bulk and the sc to match.
This makes z_bio donor-specific instead of cluster-specific. Combined with
the existing modality flip, the decoder should be able to render
donor-specific outputs.

Identical to Q24 (Run 14) otherwise.
"""
...
LAM_CYCLE = 0.3
LAM_SUP = 1.0
LAM_DONOR_ID = 5.0     # NEW: same-donor z_bio matching weight
BETA_BIO = 1e-3
...
def main():
    print(f"[Q25/Run15] device={DEVICE}  lam_donor_id={LAM_DONOR_ID}")
    ...
    for fold, (held_dn, held_ts) in enumerate(paired_dts):
        ...
        for ep in range(1, EPOCHS + 1):
            ...
            # ── NEW: same-donor z_bio matching ──
            # mu_b_b[i] (bulk_donor_d, tissue_t) should match mu_b_s[i]
            # (sn_donor_d, tissue_t) — they're paired by construction.
            ...
            if fold == 0 and (ep % 25 == 0 or ep == 1):
                print(f"    ep{ep:3d}  loss={loss.item():.3f}  recon={recon.item():.3f}  "
                      f"paired_b2s={paired_b2s.item():.3f}  donor_id={donor_id.item():.3f}",
                      flush=True)
    ...
    print(f"\n=== Run 15 LOO summary across {len(fold_results)} folds ===")
    ...
    with (OUT_DIR / "q25_eraslan_run15_donorid.json").open("w") as f:
        json.dump(out, f, indent=2)
    print(f"saved → {OUT_DIR / 'q25_eraslan_run15_donorid.json'}")
