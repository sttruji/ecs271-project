"""Q26 / Run 16 — Eraslan paired with contrastive (InfoNCE) loss on the flip.

For each paired (bulk_d, sc_d) batch element:
  query   = decode(z_bio_bulk_d, z_meta=sc)   # the flipped output
  positive = sc_d itself
  negatives = all other sc samples in the batch

InfoNCE loss: encourages the flipped output to be cosine-closer to its
matched donor's sc than to any other sc in the batch. This DIRECTLY
optimizes the Test 2 metric.

Combined with all Run 15 losses (recon, KL, sup, paired MSE, cycle, leak,
donor_id). Plus reverse-direction contrastive (sc → flip → bulk).
"""
...
LAM_SUP = 1.0
LAM_DONOR_ID = 5.0
LAM_NCE = 5.0          # NEW: contrastive weight
NCE_TAU = 0.1          # InfoNCE temperature
BETA_BIO = 1e-3
...
def info_nce_loss(query: torch.Tensor, positive: torch.Tensor, tau: float = 0.1) -> torch.Tensor:
    """InfoNCE on a batch where query[i] should match positive[i] via cosine.

    query, positive: (B, D)
    Returns the symmetric InfoNCE (q→p and p→q averaged).
    """
    q = nn.functional.normalize(query, dim=1)
    p = nn.functional.normalize(positive, dim=1)
    # Symmetric similarity matrix (B, B)
    logits_qp = q @ p.t() / tau
    logits_pq = p @ q.t() / tau
    target = torch.arange(query.size(0), device=query.device)
    return 0.5 * (nn.functional.cross_entropy(logits_qp, target)
                  + nn.functional.cross_entropy(logits_pq, target))


def main():
    print(f"[Q26/Run16] device={DEVICE}  lam_nce={LAM_NCE}  tau={NCE_TAU}")
    ...
    for fold, (held_dn, held_ts) in enumerate(paired_dts):
        ...
        for ep in range(1, EPOCHS + 1):
            ...
            # ── NEW: InfoNCE on flipped outputs ──
            # x_flip_b2s[i] should be cosine-closer to Xs_pair[i] than to Xs_pair[j!=i]
            nce_b2s = info_nce_loss(x_flip_b2s, Xs_pair, tau=NCE_TAU)
            nce_s2b = info_nce_loss(x_flip_s2b, Xb_pair, tau=NCE_TAU)
            # Also: contrastive on the LATENT z_bio side — encode(bulk) and encode(sc)
            # for the same donor should be cosine-close, others far.
            nce_z = info_nce_loss(mu_b_b, mu_b_s, tau=NCE_TAU)
            ...
            loss = (recon + BETA_BIO * kl_b + LAM_SUP * (sup_mod + sup_tis)
                    ...
                    + LAM_NCE * (nce_b2s + nce_s2b + nce_z))
            opt.zero_grad(); loss.backward(); opt.step()

            if fold == 0 and (ep % 25 == 0 or ep == 1):
                print(f"    ep{ep:3d}  loss={loss.item():.3f}  recon={recon.item():.3f}  "
                      f"paired_b2s={paired_b2s.item():.3f}  nce_b2s={nce_b2s.item():.3f}  "
                      f"nce_z={nce_z.item():.3f}  donor_id={donor_id.item():.4f}",
                      flush=True)
    ...
    print(f"\n=== Run 16 LOO summary across {len(fold_results)} folds ===")
    ...
    out = {
        "config": {"epochs": EPOCHS, "lr": LR, "lam_paired": LAM_PAIRED,
                   ...
                   "lam_donor_id": LAM_DONOR_ID, "lam_nce": LAM_NCE, "nce_tau": NCE_TAU},
    ...
    with (OUT_DIR / "q26_eraslan_run16_contrastive.json").open("w") as f:
        json.dump(out, f, indent=2)
    print(f"saved → {OUT_DIR / 'q26_eraslan_run16_contrastive.json'}")
