"""Q28 / Run 18 — Eraslan paired with donor-classifier head on z_bio.

Add a CE classifier head: z_bio -> 16-class donor prediction. This forces
z_bio to be donor-discriminative across tissues.

The held-out donor IS still seen in OTHER tissues during training (LOO is per
(donor, tissue), not per donor). So the donor classifier should generalize:
the held-out (donor X, tissue T) bulk should encode to a z_bio that classifies
to donor X, because the model learned donor X's z_bio from their other tissues.
"""
...
LAM_SUP = 1.0
LAM_DONOR_ID = 5.0
LAM_DONOR_CLF = 5.0     # NEW: donor-classifier weight
BETA_BIO = 1e-3
...
def main():
    print(f"[Q28/Run18 donor-classifier head] device={DEVICE} lam_donor_clf={LAM_DONOR_CLF}")
    ...
    donor_to_idx = {dn: i for i, dn in enumerate(donors)}
    n_donors = len(donors)
    print(f"  donors={n_donors}, tissues={len(tissues)}")
    ...
    for fold, (held_dn, held_ts) in enumerate(paired_dts):
        ...
        # Donor labels for the donor classifier
        d_b_pair_idx = torch.tensor([donor_to_idx[dn] for dn in donor_b_tr[bulk_pair_idx]],
                                    device=DEVICE, dtype=torch.long)
        d_s_pair_idx = torch.tensor([donor_to_idx[dn] for dn in donor_s_tr[sn_pair_idx]],
                                    device=DEVICE, dtype=torch.long)
        ...
        donor_clf = nn.Linear(50, n_donors).to(DEVICE)   # NEW
        opt = torch.optim.Adam(list(model.parameters()) + list(donor_clf.parameters()), lr=LR)
        ...
        for ep in range(1, EPOCHS + 1):
            ...
            # ── NEW: donor-classifier loss on z_bio ──
            logits_b = donor_clf(z_b_b)
            logits_s = donor_clf(z_b_s)
            donor_clf_loss = (nn.functional.cross_entropy(logits_b, d_b_pair_idx)
                              + nn.functional.cross_entropy(logits_s, d_s_pair_idx))
            ...
            loss = (recon + BETA_BIO * kl_b + LAM_SUP * (sup_mod + sup_tis)
                    ...
                    + LAM_DONOR_CLF * donor_clf_loss)
            opt.zero_grad(); loss.backward(); opt.step()

            if fold == 0 and (ep % 25 == 0 or ep == 1):
                with torch.no_grad():
                    pred_d_b = logits_b.argmax(dim=1)
                    acc = (pred_d_b == d_b_pair_idx).float().mean().item()
                print(f"    ep{ep:3d}  loss={loss.item():.3f}  recon={recon.item():.3f}  "
                      f"paired_b2s={paired_b2s.item():.3f}  donor_clf={donor_clf_loss.item():.3f}  "
                      f"donor_acc={acc:.2f}", flush=True)

        model.eval()
        with torch.no_grad():
            x_held_b = torch.from_numpy(bulk_x[held_bulk_i:held_bulk_i+1]).to(DEVICE)
            mu_m_h, _, mu_b_h, _ = model.encode(x_held_b)
            # Predict donor from held-out z_bio
            held_logits = donor_clf(mu_b_h)
            pred_donor_idx = held_logits.argmax(dim=1).item()
            pred_donor = donors[pred_donor_idx]
            true_donor_correct = (pred_donor == held_dn)
        ...
        print(f"  fold {fold+1}/{len(paired_dts)}: held=({held_dn}, {held_ts}); "
              ...
              f"top-1={topK[1]:.2f}; top-3={topK[3]:.2f}; "
              f"donor_clf_correct={true_donor_correct} (pred={pred_donor})", flush=True)
        ...
        fold_results.append({
            ...
            "donor_clf_pred": pred_donor,
            "donor_clf_correct": bool(true_donor_correct),
        })
    ...
    arr_donor_clf = np.array([f["donor_clf_correct"] for f in fold_results])
    ...
    print(f"\n=== Run 18 LOO summary across {len(fold_results)} folds ===")
    ...
    print(f"  donor-clf accuracy on held-out bulk: {arr_donor_clf.mean():.3f}  (random ~1/16 = 0.062)")

    out = {
        "config": {"epochs": EPOCHS, "lr": LR, "lam_paired": LAM_PAIRED,
                   "lam_donor_clf": LAM_DONOR_CLF},
        ...
        "summary": {
            ...
            "donor_clf_accuracy": float(arr_donor_clf.mean()),
        },
    }
    with (OUT_DIR / "q28_eraslan_run18_donorhead.json").open("w") as f:
        json.dump(out, f, indent=2)
    print(f"saved → {OUT_DIR / 'q28_eraslan_run18_donorhead.json'}")
